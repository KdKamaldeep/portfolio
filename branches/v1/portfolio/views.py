...
@login_required(login_url=settings.LOGIN_URL) #Deprecated
def viewportfolio(request, portfolioid):
    protocol = 'http'
    if request.is_secure():
        protocol = 'https'
    portfoio = portmodel.portFolio.objects.get(user__id=request.user.id, uuid=portfolioid)
    url = '{2}://{0}/portfolio/view/{1}'.format(request.META['HTTP_HOST'], portfoio.uuid, protocol)
    return render_to_response('portfolio/view-frame.html', {'request':request,'url': url});